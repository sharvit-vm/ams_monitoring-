            ...
            <Panel title="Execution Workflow">
              {workflow.isError ? (
                <Alert severity="warning" sx={{ mb: 1 }}>
                  The workflow metadata could not be loaded from the backend. Check that the gateway is running and reachable.
                </Alert>
              ) : null}
              <Box className="flow-wrap">
                <Box sx={{ width: '100%', height: '100%' }}>
                  <ReactFlow
                    ...
                    fitView
                    onNodeClick={(_, node) => {
                      const instance = (node.data as any)?.instance_url;
                      if (node.id === 'servicenow' && instance) {
                        try {
                    ...
                    onNodeMouseEnter={(event, node) => {
                      ...
                      if (node.id === 'servicenow') {
                        const url = instance ? String(instance) : 'ServiceNow instance URL not configured';
                        setHoverUrl(url);
                        setHoverPos({ x: event.clientX + 12, y: event.clientY + 12 });
                      }
                    }}
                    onNodeMouseMove={(event) => {
                      if (hoverUrl) setHoverPos({ x: event.clientX + 12, y: event.clientY + 12 });
                    }}
                    onNodeMouseLeave={() => {
                      setHoverUrl(null);
                      setHoverPos(null);
                ...
                {hoverUrl && hoverPos && (
                  <Box className="node-tooltip" sx={{ position: 'fixed', left: hoverPos.x, top: hoverPos.y, zIndex: 1300, bgcolor: 'background.paper', px: 1, py: 0.5, borderRadius: 1, boxShadow: 3, fontSize: 12 }}>
                    {hoverUrl}
                  </Box>
                )}
              </Box>
